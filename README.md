...
- opencode: registered via `plugin: ["context-mode"]` in `opencode/.config/opencode/opencode.json`. Plugin is loaded in-process — no MCP entry needed. Routing rules live in `opencode/.config/opencode/AGENTS.md`.
- Claude Code and Codex: **not installed**. The plugin, its marketplace, and the `context-mode-cache-heal.mjs` `SessionStart` hook were removed from `claude/.claude/settings.json` and `~/.codex/config.toml`.
