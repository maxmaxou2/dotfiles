...
- Rename a poorly named variable → triggers `py-clean-names`
- Delete a redundant comment → triggers `py-clean-comments`
- Remove dead code or unused imports
- Replace a magic number with a named constant
- Extract a deeply nested block into a well-named function

### Deeper Improvements (When Time Allows)
- Split a function that does multiple things → triggers `py-clean-functions`
- Remove duplication (DRY) → triggers `py-clean-general`
- Add missing boundary checks
- Improve test coverage → triggers `py-clean-tests`
...
| Naming variables, functions, classes | `py-clean-names` |
| Writing or editing comments | `py-clean-comments` |
| Creating or refactoring functions | `py-clean-functions` |
| Reviewing code quality | `py-clean-general` |
| Writing or reviewing tests | `py-clean-tests` |
